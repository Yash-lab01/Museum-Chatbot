...
# ─── History Trimmer ─────────────────────────────────────────────
# Keeps the last N messages to prevent context window overflow.
# Uses len (message count) as the token counter.
# start_on="human" ensures we never split a tool-call/tool-response pair.
MAX_HISTORY_MESSAGES = 20

def prompt_with_trimming(state):
    """Trims conversation history and prepends the system prompt."""
...
def get_agent():
    ...
    # Create the langgraph agent with trimmed history
